document.addEventListener("DOMContentLoaded", () => {
  ...
  for (const [key, pokedex] of Object.entries(events)) {
    let regionBreaks = key === "gofest" ? REGION_BREAKS_GOFEST : REGION_BREAKS_STANDARD;
    insertRegionTitles(pokedex, regionBreaks);
    pokedex.regionBreaks = regionBreaks;
  }
  ...
  app.innerHTML = `
    <nav class="navbar">
      ...
      <div class="navbar-links">
        ...
        <button class="nav-btn" data-section="tcg">
    ...
    <div id="tcg" class="section" style="display:none;">
      <div class="hero-section">
        <h1 class="main-title">Pokemon TCG Pocket</h1>
        <p class="hero-subtitle">Collect every card in the digital TCG experience</p>
      </div>
      
      <div class="collections">
        ${Object.entries(tcgSets).map(([key, set]) => `
          <div class="collection-card" onclick="renderCollection('${key}', 'all', 'tcg')">
            <div class="card-glow"></div>
            <div class="card-icon">🃏</div>
            <div class="card-content">
              <h2>${set.title}</h2>
    ...
    <div id="cards" class="section" style="display:none;">
      <div class="hero-section">
        <h1 class="main-title">Pokemon Trading Cards</h1>
        <p class="hero-subtitle">Classic card collection from the base set series</p>
      </div>
      
      <div class="collections">
        ${Object.entries(cardSets).map(([key, set]) => `
          <div class="collection-card" onclick="renderCollection('${key}', 'all', 'cards')">
            <div class="card-glow"></div>
            <div class="card-icon">🎴</div>
            <div class="card-content">
              <h2>${set.title}</h2>
...
function renderCollection(key, filter = "all", type = "pogo") {
  ...
  if (type === "tcg") {
    ...
    collection = pokedexes[key] || medals[key] || events[key];
  }
...
function downloadPDF(key) {
  const collection = pokedexes[key] || medals[key] || events[key] || tcgSets[key] || cardSets[key];
